...
class SurrealDBWhereClauseGenerator
  ...
  StringBuffer _generateOperatorMethod({
    ...
    StringBuffer buildValue(
      DartType type,
      String parameterName,
      bool supportsDynamicType,
    ) {
      final stringBuffer = StringBuffer();
      stringBuffer.writeln('dynamic __valueToCompare = $parameterName;');
      // if the value supports dynamic type,
      // do apply the typeChecking on the runtime
      // else check the type right now itself.
      if (supportsDynamicType) {
        stringBuffer
          // as at runtime, we don't know what kind of enum is sent
          // so simply use the name property available on enum.
          ..writeln('if (__valueToCompare is Enum) {')
          ..writeln('__valueToCompare = __valueToCompare.name;')
          ..writeln('}');
      } else if (type.isEnum) {
